...
def QueryFunc(addr, insertnum):
  contentsrc = "objids_rep3.txt"
  f = open(contentsrc, "r")
  mm = mmap.mmap(f.fileno(), 0, prot=mmap.PROT_READ)
  fsize = os.path.getsize(contentsrc)
  numtopicids = fsize/24
  #client = MongoClient(addr, 30001)
  client = MongoReplicaSetClient(addr+":30001", replicaset="rs3")
  db = client.topicdbreplshard
  ...
  while count < totinsertnum:
    ...
    #collection.find_one({"_id":ObjectId(rand_topicid)}, read_preference=ReadPreference.SECONDARY)
