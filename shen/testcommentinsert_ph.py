...
def GenerateTopicPost(mm, fsize):
  ...
  topicpost={
        ...
        "commentList":[{
          "id": 0,
          "userid": 0,
          "content": mm[contentstart:contentstart+100],
          "createtime": datetime.datetime.now(),
          "parentid": 0
        }],
        "favCount":0,
...
def InsertCommentFunc(collection, i, insertnum, mm, fsize):
  count = 0
  while count < insertnum:
    cursor = collection.find({"teamid":random.randint(0, 100000)})
    for c in cursor:
#   print "insert teamid:", c["teamid"], "   id:", c["_id"]
      ...
      collection.update({"_id":c["_id"]}, {"$push": {"commentList": comment}})
    count += 1

def RefreshTimeFunc(collection, i, insertnum, mm, fsize):
  count = 0
  while count < insertnum:
    cursor = collection.find({"teamid":random.randint(0, 100000)})
    for c in cursor:
#   print "insert teamid:", c["teamid"], "   id:", c["_id"]
      collection.update({"_id":c["_id"]}, {"$set": {"refreshtime": datetime.datetime.now()}})
      break
    count += 1
...
#    threads.append(curthread)
  ...
  for i in range(threadnum):
    curthread = Process(target=RefreshTimeFunc, args=(collection, i, insertnum, mm1, fsize1))
    curthread.start()
    threads.append(curthread)
